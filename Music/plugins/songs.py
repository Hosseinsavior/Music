...
async def songs(_, message: Message):
    ...
    try:
        ...
        return await hell.edit_text(f"An error occurred: {e}")
...
async def lyrics(_, message: Message):
    ...
    song = query[0].strip()
    artist = query[1].strip() if len(query) == 2 else ""
    ...
    if results:
        ...
        await hell.edit_text("Unexpected error occurred.")
...
async def song_cb(_, cb: CallbackQuery):
    ...
    try:
        if action in ["adl", "vdl"]:
            await ytube.send_song(cb, rand_key, key, action == "vdl")
        else:
