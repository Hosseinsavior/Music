...
async def controls_cb(_, cb: CallbackQuery):
    try:
        video_id, chat_id = cb.data.split("|")[1:3]
        btns = Buttons.controls_markup(video_id, int(chat_id))
        await cb.message.edit_reply_markup(InlineKeyboardMarkup(btns))
    except Exception as e:
        print(f"Error in controls_cb: {e}")
...
async def player_cb(_, cb: CallbackQuery):
    try:
        ...
        btns = Buttons.player_markup(int(chat_id), video_id, hellbot.app.username)
        await cb.message.edit_reply_markup(InlineKeyboardMarkup(btns))
    except Exception as e:
        print(f"Error in player_cb: {e}")
...
async def controler_cb(_, cb: CallbackQuery):
    try:
        ...
        if not is_authchat and cb.from_user.id not in Config.SUDO_USERS:
            try:
                admins = await get_auth_users(chat_id)
                if cb.from_user.id not in admins:
                    return await cb.answer("This command is only for authorized users and admins!", show_alert=True)
            except Exception as e:
                return await cb.answer(f"There was an error while fetching admin list.\n\n{e}", show_alert=True)
        ...
        if action == "play":
            ...
            if is_paused:
                ...
                return await cb.message.reply_text(f"__VC Resumed by:__ {cb.from_user.mention}")
            else:
                ...
                return await cb.message.reply_text(f"__VC Paused by:__ {cb.from_user.mention}")
            ...
            if is_muted:
                ...
                return await cb.message.reply_text(f"__VC Muted by:__ {cb.from_user.mention}")
            ...
            if is_muted:
                ...
                return await cb.message.reply_text(f"__VC Unmuted by:__ {cb.from_user.mention}")
            else:
            ...
            que = Queue.get_queue(cb.message.chat.id)
            video_id = "telegram" if not que else que[0]["video_id"]
        ...
        print(f"Error in controler_cb: {e}")
